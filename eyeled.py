...
try:
    while True:
        ...
        if results.multi_face_landmarks:
            ...
            # Determine eye status and control LED
            if avg_ear < EAR_THRESHOLD:
                eye_status = "Closed"
                lgpio.gpio_write(h, LED_PIN, 1)  # LED ON when eyes closed
                print("Eyes Closed - LED ON")
            else:
                eye_status = "Open"
                lgpio.gpio_write(h, LED_PIN, 0)  # LED OFF when eyes open
                print("Eyes Open - LED OFF")
